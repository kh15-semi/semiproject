...
//		}
		 ...
		 String sql = "SELECT * FROM ("
	                ...
	                + "SELECT "
	                + "    review_no, review_writer, review_company_no, review_score, review_comment, review_wtime, review_etime, review_like, "
	                + "    review_salary, review_work_and_life, review_promotion, review_culture, review_director, "
	                + "    member_name, member_industry, member_job "
	                + "FROM review_list_view "
	                + "WHERE review_company_no = ? "
	                + "ORDER BY review_no DESC"
	                + ") TMP"
	                + ") WHERE RN BETWEEN ? AND ?";
	        Object[] data = {
    ...
    private boolean isValidColumnName(String column) {
        // 허용되는 컬럼 이름 목록
        List<String> validColumns = List.of("review_no", "review_writer", "review_comment", "member_name", "member_industry", "member_job",
                "review_salary", "review_work_and_life", "review_promotion", "review_culture", "review_director");

...
<!-- 이전 -->
<c:if test="${pageVO.hasPrevBlock()}">
    <a href="detail?page=${pageVO.prevBlock}&companyNo=${companyNo}">
        &lt;
    </a>
</c:if>
...
<c:forEach var="i" begin="${pageVO.startBlock}" end="${pageVO.finishBlock}" step="1">
    <c:choose>
        ...
        <c:otherwise>
            <a href="detail?page=${i}&companyNo=${companyNo}">${i}</a>
        </c:otherwise>
    </c:choose>
</c:forEach>
...
<!-- 다음 -->
<c:if test="${pageVO.hasNextBlock()}">
    <a href="detail?page=${pageVO.nextBlock}&companyNo=${companyNo}">
        &gt;
    </a>
</c:if>
